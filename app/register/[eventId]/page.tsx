...
import {
  ArrowLeft, Check, Upload, X, MapPin, Calendar,
  Ticket, Loader2, Plus, ShieldCheck, AlertCircle, Lock,
  CalendarClock, Clock, Building2, Users
} from 'lucide-react'
...
async function uploadEventFile(file: File, eventId: string): Promise<string> {
  ...
  const { data: { publicUrl } } = supabase.storage.from('event-uploads').getPublicUrl(data.path)
  return publicUrl
}
  ...
  return (
    <div className="border-b border-[#ececec] last:border-b-0">
      ...
      >
        ...
        <span className={`shrink-0 w-6 h-6 rounded-full bg-[#f2f1fb] text-[#5b4fe5] flex items-center justify-center transition-transform duration-300 ${open ? 'rotate-45' : ''}`}>
          <Plus size={13} strokeWidth={2.5} />
        </span>
      </button>
      <div className={`grid transition-all duration-300 ease-out ${open ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
        <div className="overflow-hidden">
          <div className="pb-5 text-sm text-[#68686e] leading-relaxed">{children}</div>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex items-start gap-3.5">
      ...
      <div className="pt-1.5">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-[#9a9aa2]">{label}</p>
...
function formatEventSchedule(event: EventDef): { day: string; time: string } | null {
  if (!event.eventStartDate) return null
  try {
    const start = new Date(event.eventStartDate)
    const end = event.eventEndDate ? new Date(event.eventEndDate) : null
    const day = start.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
    ...
    if (end) {
      ...
      return { day, time: sameDay ? `${startTime} – ${endTime}` : `${startTime} onwards, multi-day` }
    }
...
export default function EventRegisterPage() {
  ...
  const [photoPreviews, setPhotoPreviews] = useState<Record<string, { name: string; preview: string }>>({})
  ...
  useEffect(() => {
    return () => { Object.values(photoPreviews).forEach((p) => URL.revokeObjectURL(p.preview)) }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const setValue = (fieldId: string, v: string | string[]) => setValues((prev) => ({ ...prev, [fieldId]: v }))

  const toggleCheckbox = (field: FieldDef, optId: string) => {
    const current = (values[field.id] as string[] | undefined) ?? []
    setValue(field.id, current.includes(optId) ? current.filter((x) => x !== optId) : [...current, optId])
  }

  const onFileChange = (fieldId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFileErrors((prev) => { const copy = { ...prev }; delete copy[fieldId]; return copy })
    setPhotoPreviews((prev) => {
      if (prev[fieldId]) URL.revokeObjectURL(prev[fieldId].preview)
      return { ...prev, [fieldId]: { name: file.name, preview: URL.createObjectURL(file) } }
    })
  ...
  const removeSelectedFile = (fieldId: string) => {
    setPhotoPreviews((p) => {
      ...
      const copy = { ...p }; delete copy[fieldId]; return copy
    })
    setRawFiles((rf) => { const copy = { ...rf }; delete copy[fieldId]; return copy })
    setFileErrors((fe) => { const copy = { ...fe }; delete copy[fieldId]; return copy })
    setValue(fieldId, '')
  }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (availability !== 'open') {
      setError(availability === 'upcoming'
        ? `Registrations open on ${formatShortDate(event.startDate)}.`
        : 'Registrations for this event are currently closed.')
      return
    }
    if (!acceptedTerms) {
      setError('You must accept the Event Rules and Terms & Conditions to proceed.')
      return
    }
    ...
    for (const field of customerFields) {
      ...
      const empty = field.type === 'checkbox-group' ? !v || (v as string[]).length === 0 : !v
      if (empty) {
    ...
    try {
      ...
      event.fields.filter((f) => f.filledBy === 'preset').forEach((f) => { finalValues[f.id] = f.presetValue ?? '' })

      const res = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ eventId: event.id, ticketId: selectedTicketId, values: finalValues })
      })
      ...
      if (orderData.isFree || orderData.alreadyConfirmed) {
        if (!orderData.registrationId) {
          setError('Registration created but confirmation could not be loaded. Please contact support.')
          setIsSubmitting(false)
          return
        }
        router.push(`/registration-confirmation?rid=${orderData.registrationId}`)
        return
      }
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            if (verifyRes.ok && verifyJson.success && verifyJson.registrationId) {
              router.push(`/registration-confirmation?rid=${verifyJson.registrationId}`)
              return
            }
            setError('Payment verification failed. If money was deducted, it will be automatically refunded.')
          } catch {
            setError('Network error during verification. We will reconcile this shortly.')
          } finally {
            setIsSubmitting(false)
          }
        },
      ...
      rzp.on('payment.closed', function () { setIsSubmitting(false) })
      rzp.open()

    } catch (err: any) {
      console.error(err)
      setError('Unable to complete registration. Please check your file upload and try again.')
      setIsSubmitting(false)
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#fafafa] text-[#68686e] gap-3">
        ...
        <p className="text-sm font-medium tracking-wide">Loading event details…</p>
      </div>
    )
  }
  ...
  const totalSold = event.tickets.reduce((sum, t) => sum + (claims[t.id] || 0), 0)
      ...
      <div className="relative h-[38vh] min-h-[260px] max-h-[420px] w-full overflow-hidden">
        ...
        {availability !== 'open' && (
          <span className="absolute top-5 right-5 sm:top-6 sm:right-6 text-[10px] font-bold uppercase tracking-wider bg-white/95 text-[#c0392b] px-3 py-1.5 rounded-full shadow-sm">
            {availability === 'upcoming' ? 'Opening Soon' : event.status === 'draft' ? 'Draft Mode' : 'Registration Closed'}
          </span>
        )}
      </div>
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-6 lg:gap-8 items-start">
          ...
          <div className="bg-white rounded-3xl shadow-[0_20px_50px_-20px_rgba(23,23,26,0.15)] border border-[#ececec] p-6 sm:p-9 space-y-8">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              {schedule && <InfoRow icon={Calendar} label="When" value={<>{schedule.day}<br /><span className="text-[#68686e] font-normal">{schedule.time}</span></>} />}
              {event.address && <InfoRow icon={MapPin} label="Where" value={event.address} />}
              ...
              {totalCapacity > 0 && (
                <InfoRow icon={Users} label="Spots" value={`${Math.max(totalCapacity - totalSold, 0)} of ${totalCapacity} remaining`} />
              )}
            </div>

            {event.description && (
              <div className="pt-2 border-t border-[#ececec]">
                <h2 className="text-sm font-bold text-[#17171a] mb-2 pt-6">About this event</h2>
                <p className="text-sm text-[#68686e] leading-relaxed whitespace-pre-wrap">{event.description}</p>
              </div>
            )}
            ...
            {event.rules && event.rules.length > 0 && (
              <div className="pt-2 border-t border-[#ececec]">
                <ExpandSection
                  title={<span className="inline-flex items-center gap-2"><ShieldCheck size={15} className="text-[#5b4fe5]" /> Rules &amp; Regulations</span>}
                >
            ...
            <div className="pt-2 border-t border-[#ececec]">
              <h2 className="text-xs font-bold uppercase tracking-wider text-[#9a9aa2] mb-1">Policies &amp; Terms</h2>
              {[
                { title: 'Terms & Conditions', content: event.termsAndConditions },
                { title: 'Privacy Policy', content: event.privacyPolicy },
                { title: 'Refund & Cancellation Policy', content: event.refundPolicy },
                { title: 'Shipping & Delivery Policy', content: event.shippingPolicy },
              ].map((policy, pIdx) => policy.content ? (
          ...
          <div className="lg:sticky lg:top-6">
            ...
            >
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center justify-between p-3.5 rounded-2xl border-2 transition-all duration-200 ${
                          isSoldOut || !isEventOpen ? 'cursor-not-allowed opacity-40 bg-[#f7f7f8] border-transparent' :
                          isSelected ? 'border-[#5b4fe5] bg-[#f2f1fb] cursor-pointer' : 'border-[#ececec] hover:border-[#c9c4f7] cursor-pointer'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <input
                            type="radio" disabled={!isEventOpen || isSoldOut} name="event_tkt" value={tkt.id}
                            checked={isSelected} onChange={(e) => setSelectedTicketId(e.target.value)}
                            className="w-4 h-4 accent-[#5b4fe5] cursor-pointer disabled:cursor-not-allowed"
                          />
                          <div>
                            <div className="text-sm font-bold text-[#17171a]">{tkt.name}</div>
                            {stockLeft !== null && !isSoldOut && stockLeft <= 20 && (
                              <div className="text-[10px] font-bold text-[#c9820a] mt-0.5">{stockLeft} left</div>
                            )}
                            {isSoldOut && <div className="text-[10px] font-bold text-[#c0392b] mt-0.5">Sold out</div>}
                          </div>
                        </div>
                ...
                {customerFields.map((field) => (
                  <div key={field.id}>
                    <label className="block text-xs font-bold text-[#17171a] mb-1.5">
                      {field.label} {field.required && <span className="text-[#5b4fe5]">*</span>}
                    </label>

                    {['text', 'number', 'email', 'date'].includes(field.type) && (
                      <input
                        type={field.type} disabled={!isEventOpen} required={field.required} placeholder={field.placeholder}
                        value={(values[field.id] as string) ?? ''} onChange={(e) => setValue(field.id, e.target.value)}
                    ...
                    {field.type === 'phone' && (
                      <div className="flex rounded-xl border border-[#e2e2e5] overflow-hidden focus-within:border-[#5b4fe5] focus-within:ring-4 focus-within:ring-[#5b4fe5]/10 bg-[#fafafa] focus-within:bg-white transition-all">
                        <span className="flex items-center px-3 text-xs font-bold text-[#9a9aa2] border-r border-[#e2e2e5]">+91</span>
                        <input
                          type="tel" disabled={!isEventOpen} required={field.required} placeholder={field.placeholder ?? '98765 43210'}
                          value={(values[field.id] as string) ?? ''} onChange={(e) => setValue(field.id, e.target.value)}
                    ...
                    {field.type === 'textarea' && (
                      <textarea
                        disabled={!isEventOpen} required={field.required} rows={3}
                        value={(values[field.id] as string) ?? ''} onChange={(e) => setValue(field.id, e.target.value)}
                        className="w-full rounded-xl border border-[#e2e2e5] px-3.5 py-2.5 text-sm outline-none focus:border-[#5b4fe5] focus:ring-4 focus:ring-[#5b4fe5]/10 bg-[#fafafa] focus:bg-white transition-all"
                      />
                    )}
                    ...
                    {field.type === 'select' && (
                      <select
                        disabled={!isEventOpen} required={field.required}
                        value={(values[field.id] as string) ?? ''} onChange={(e) => setValue(field.id, e.target.value)}
                    ...
                    {(field.type === 'radio' || field.type === 'checkbox-group') && (
                      <div className="space-y-2 mt-1">
                        {field.options?.map((opt) => {
                          ...
                          const checked = isMulti ? ((values[field.id] as string[] | undefined) ?? []).includes(opt.id) : values[field.id] === opt.id
                          return (
                            <label key={opt.id} className={`flex items-center gap-2.5 rounded-xl border p-2.5 text-xs transition-colors cursor-pointer ${
                              checked ? 'border-[#5b4fe5] bg-[#f2f1fb] text-[#17171a] font-bold' : 'border-[#e2e2e5] hover:border-[#c9c4f7]'
                            }`}>
                              <input
                                type={isMulti ? 'checkbox' : 'radio'} disabled={!isEventOpen} name={field.id} checked={checked}
                                onChange={() => (isMulti ? toggleCheckbox(field, opt.id) : setValue(field.id, opt.id))}
                    ...
                    {field.type === 'file' && (
                      <div>
                        <label className="flex items-center justify-center gap-2 rounded-xl border-2 border-dashed border-[#e2e2e5] p-3.5 text-xs font-medium text-[#68686e] cursor-pointer hover:border-[#5b4fe5]/50 hover:bg-[#f2f1fb] bg-[#fafafa] transition-colors">
                          ...
                          <span className="truncate">{photoPreviews[field.id]?.name ?? 'Upload PNG/JPEG, max 50KB'}</span>
                          <input
                            type="file"
                        ...
                        {fileErrors[field.id] && (
                          <p className="mt-1.5 text-[10px] font-semibold text-[#c0392b] flex items-center gap-1">
                            <AlertCircle size={11} className="shrink-0" /> {fileErrors[field.id]}
                          </p>
                        )}
                        {photoPreviews[field.id] && !fileErrors[field.id] && (
                          <div className="mt-2 relative inline-block">
                            <img src={photoPreviews[field.id].preview} alt="Preview" className="h-14 w-14 rounded-lg object-cover border border-[#e2e2e5]" />
                            <button type="button" onClick={() => removeSelectedFile(field.id)}
              ...
              <label className="flex items-start gap-2.5 cursor-pointer select-none pt-2 border-t border-[#ececec]">
                <input type="checkbox" disabled={!isEventOpen} checked={acceptedTerms} onChange={(e) => setAcceptedTerms(e.target.checked)}
                  className="mt-0.5 w-4 h-4 accent-[#5b4fe5] cursor-pointer" />
                <span className="text-[11px] text-[#68686e] leading-relaxed">
                  I agree to the event <strong className="text-[#17171a]">Rules</strong>, <strong className="text-[#17171a]">Terms</strong> and <strong className="text-[#17171a]">Privacy Policy</strong>.
                </span>
              </label>
              ...
              <div className="pt-2 border-t border-[#ececec] flex items-center justify-between">
                <span className="text-xs font-medium text-[#9a9aa2]">Total</span>
                <span className="text-xl font-bold text-[#17171a]">{amountToPay === 0 ? 'Free' : `₹${amountToPay}`}</span>
              </div>

              <button
                type="submit" disabled={isSubmitting || !selectedTicketId || !acceptedTerms || !isEventOpen}
              ...
              >
                {isSubmitting ? (<><Loader2 size={16} className="animate-spin" /> Processing…</>)
                  : availability === 'upcoming' ? 'Not Open Yet'
                  : !isEventOpen ? 'Registrations Closed'
                  : (amountToPay === 0 ? 'Reserve Free Spot' : `Pay ₹${amountToPay}`)}
              </button>
            </form>
          </div>
